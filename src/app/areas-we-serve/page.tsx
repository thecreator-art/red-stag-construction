      ...
      <section className="py-24 bg-warm-white">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-text-dark mb-6">Serving Greater Los Angeles</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-6xl mx-auto">
            {/* Tier 1 */}
            <div className="bg-white p-8 border-t-4 border-accent-red shadow-sm">
              <h3 className="text-2xl font-serif font-bold text-text-dark mb-6 pb-4 border-b border-gray-100">Westside & Luxury Homes</h3>
              <ul className="space-y-4 font-semibold">
                {topTierCities.map(city => (
                  <li key={city}>
                    <Link href={`/${formatSlug(city)}`} className="text-text-body hover:text-accent-red transition-colors flex items-center group">
                      <span className="w-1.5 h-1.5 bg-accent-red rounded-full mr-3 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                      {city}
                    </Link>
                  </li>
                ))}
            ...
            <div className="bg-white p-8 border-t-4 border-primary-dark shadow-sm">
              <h3 className="text-2xl font-serif font-bold text-text-dark mb-6 pb-4 border-b border-gray-100">San Fernando Valley</h3>
              <ul className="space-y-4 font-semibold">
                {svfCities.map(city => (
                  <li key={city}>
                    <Link href={`/${formatSlug(city)}`} className="text-text-body hover:text-accent-red transition-colors flex items-center group">
            ...
            {/* Tier 3 */}
            <div className="bg-white p-8 border-t-4 border-accent-red shadow-sm">
              <h3 className="text-2xl font-serif font-bold text-text-dark mb-6 pb-4 border-b border-gray-100">Greater LA & Metro</h3>
              <ul className="space-y-4 font-semibold">
