...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (location) return {
    title: `General Contractor in ${location.city}, CA | Red Stag Construction`,
    description: `Licensed general contractor in ${location.city}, CA. Building custom homes, ADUs, and high-end remodels. Free design-build estimates.`,
    alternates: { canonical: `${baseUrl}/${slug}` }
  };
  
  const matrix = matrixData.find(m => m.slug === slug);
  if (matrix) return {
    title: `${matrix.title} | Red Stag Construction`,
    description: matrix.intro.substring(0, 155).trim() + '...',
    alternates: { canonical: `${baseUrl}/${slug}` }
  };
  const blog = blogsData.find(b => b.slug === slug);
  if (blog) return {
    title: blog.title,
    description: `Read more about ${blog.keyword} and expert construction insights.`,
    alternates: { canonical: `${baseUrl}/${slug}` }
  };
...
export default async function DynamicSlugPage({ params }: PageProps) {
      ...
      <>
        ...
        <section className="bg-primary-dark py-24 border-t border-gray-800">
           <div className="container mx-auto px-4 max-w-4xl text-center">
             <h2 className="text-4xl font-serif font-bold text-white mb-6">Ready to Start Your {service.title.split(' ')[0]} Project?</h2>
    ...
    return (
      <>
        <ParallaxHero 
          ...
          imageAlt={`General Contractor ${location.city}`}
          h1Text={`General Contractor in ${location.city}, CA`}
          h2Text={`Luxury remodels, ADUs, and Custom Homes in ${location.city}.`}
          ctaText="Get a Free Estimate"
          ctaHref="/contact"
          phoneNumber="(626) 652-2303"
        />
        ...
        <section className="bg-white py-24">
          <div className="container mx-auto px-4 max-w-4xl">
             <h2 className="text-3xl font-serif font-bold text-primary-dark mb-8">Building in {location.city}</h2>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-center pt-8 border-t border-gray-200">
                <Button href={`/kitchen-remodel-${location.slug.replace('general-contractor-', '')}`} variant="outline">Kitchen Remodeling in {location.city}</Button>
                <Button href={`/bathroom-remodel-${location.slug.replace('general-contractor-', '')}`} variant="outline">Bathroom Remodeling in {location.city}</Button>
                <Button href={`/adu-contractor-${location.slug.replace('general-contractor-', '')}`} variant="outline">ADU Contractor in {location.city}</Button>
                <Button href={`/custom-home-builder-${location.slug.replace('general-contractor-', '')}`} variant="outline">Custom Homes in {location.city}</Button>
             </div>
          </div>
        </section>
        ...
        <section className="bg-primary-dark py-24 border-t border-gray-800">
           <div className="container mx-auto px-4 max-w-4xl text-center">
             <h2 className="text-4xl font-serif font-bold text-white mb-6">Start Your {location.city} Project</h2>
  ...
  if (matrix) {
    return (
      <>
        <ParallaxHero 
          imageSrc={`/images/services/${matrix.service.toLowerCase().split(' ')[0]}.jpg`} // Fallback
          imageAlt={`${matrix.service} in ${matrix.city}`}
          h1Text={matrix.title}
          h2Text={`${matrix.city}, CA`}
          ctaText="Get a Free Estimate"
          ctaHref="/contact"
          phoneNumber="(626) 652-2303"
        />
        ...
        <section className="bg-cream py-24">
          <div className="container mx-auto px-4 max-w-4xl">
            <div className="flex items-center space-x-4 mb-8">
              ...
              <h2 className="text-2xl md:text-3xl font-serif font-bold text-primary-dark">Serving {matrix.city} and Surrounding Areas</h2>
            </div>
        ...
        <section className="bg-white py-24 border-t border-gray-200">
           <div className="container mx-auto px-4 max-w-4xl text-center">
             <h2 className="text-4xl font-serif font-bold text-primary-dark mb-6">Expert {matrix.service} Installation</h2>
